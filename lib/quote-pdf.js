...
import { DATOS_BANCARIOS, FORMA_PAGO_DEFAULT } from './empresa-config.js';
...
export async function buildQuotePdf(q, opts = {}) {
  ...
  doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(...DARK);
  doc.text('Neto', boxX + 5, y + 5);
  doc.text(formatCLP(neto), W - M - 5, y + 5, { align: 'right' });
  doc.text('IVA (19%)', boxX + 5, y + 11);
  doc.text(formatCLP(iva), W - M - 5, y + 11, { align: 'right' });
  y += 15;
  ...
  doc.setFontSize(13);
  doc.text(formatCLP(totalConIva), boxX + boxW - 5, y + 9, { align: 'right' });
  y += 18;

  if (opts.ufValue && totalConIva) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...GRAY);
    doc.text(`Equivale a ${(totalConIva / opts.ufValue).toLocaleString('es-CL', { maximumFractionDigits: 1 })} UF  ·  UF ${formatCLP(opts.ufValue)}`, W - M, y, { align: 'right' });
    y += 6;
  }

  // ── Condiciones ──
  y += 4;
  const validez = q.validezDias || opts.validezDias || 30;
  const formaPago = (q.formaPago || FORMA_PAGO_DEFAULT || '').trim();
  doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(...GRAY);
  doc.text('CONDICIONES', M, y); y += 5;
  doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...DARK);
  const cond = [
    `• Cotización válida por ${validez} días desde la fecha de emisión.`,
    '• Valores netos en pesos chilenos (CLP). El total incluye IVA (19%).' + (opts.ufValue ? ' Conversión UF referencial del día.' : ''),
  ];
  if (formaPago) cond.push(`• Forma de pago: ${formaPago}`);
  cond.forEach(c => { const lines = doc.splitTextToSize(c, CW); doc.text(lines, M, y); y += lines.length * 5; });

  // ── Datos bancarios (si están configurados en lib/empresa-config.js) ──
  const bank = DATOS_BANCARIOS || {};
  if (bank.numero && (bank.banco || bank.titular)) {
    if (y > 252) { doc.addPage(); y = 20; }
    y += 6;
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(...GRAY);
    doc.text('DATOS PARA TRANSFERENCIA', M, y); y += 5;
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...DARK);
    [
      bank.titular && `Titular: ${bank.titular}`,
      bank.rut && `RUT: ${bank.rut}`,
      bank.banco && `Banco: ${bank.banco}`,
      bank.tipoCuenta && `Cuenta: ${bank.tipoCuenta}`,
      bank.numero && `N°: ${bank.numero}`,
      bank.email && `Email: ${bank.email}`,
    ].filter(Boolean).forEach(line => { doc.text(line, M, y); y += 5; });
  }
